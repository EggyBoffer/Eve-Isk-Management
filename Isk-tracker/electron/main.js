...
function createOverlay() {
  ...
  globalShortcut.register('Escape', () => {
    overlayWin.close();
    globalShortcut.unregister('Escape'); // cleanup when closed
  });

  overlayWin.on('closed', () => {
    globalShortcut.unregister('Escape'); // also cleanup
  });

  overlayWin.on('close', () => {
    const bounds = overlayWin.getBounds();
    saveSettings(bounds); // save x/y/width/height on close
  });

  return overlayWin;
}

// Listen for the request to open overlay
...
ipcMain.handle('update-entry', (event, category, entry) => {
  ...
  const stmt = db.prepare(`UPDATE ${category} SET date=?, room1_isk=?, room2_isk=?, room3_isk=?, time_taken=?, fillament_cost=? WHERE id=?`);
