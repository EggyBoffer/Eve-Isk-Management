...
export default function DEDAnalytics() {
  ...
  const [dedFilter, setDedFilter] = useState("All"); // <-- NEW

  useEffect(() => {
    setRuns(loadRuns());
    const onStorage = (e) => { if (e.key === "ded:runs:v1") setRuns(loadRuns()); };
    window.addEventListener("storage", onStorage);
    return () => window.removeEventListener("storage", onStorage);
  }, []);

  // Unique available DED levels from data
  ...
  return (
    <div className="ded-analytics-page">
      <div className="ded-analytics-header">
        ...
        {/* NEW: DED level filter */}
        <div className="ded-filter-row" style={{ display: "flex", gap: "0.75rem", alignItems: "center", marginTop: "0.5rem" }}>
          <label style={{ fontSize: "0.95rem", opacity: 0.9 }}>Filter by DED:</label>
          <select
            value={dedFilter}
          ...
          <span className="ded-muted">
            Scope: <strong>{dedFilter === "All" ? "All levels" : dedFilter}</strong> — {fmtInt(stats.totalRuns)} run{stats.totalRuns === 1 ? "" : "s"}
          </span>
        </div>
      </div>
      ...
      {/* Per-level table (always computed from filteredRuns, so it narrows too) */}
      <section className="ded-section">
        <div className="ded-section-head">
          <h2>By DED Level</h2>
          <small className="ded-muted">
            Based on {fmtInt(stats.totalRuns)} run{stats.totalRuns === 1 ? "" : "s"} in current scope.
          </small>
        </div>
          ...
          <table className="ded-table">
            ...
            <tbody>
              ...
              {stats.byLevel.length === 0 && (
                <tr><td colSpan={5} className="ded-muted">No data yet.</td></tr>
              )}
            </tbody>
          </table>
        </div>
      ...
      {/* Top items by value (drop rate now uses filteredRuns denominator) */}
      <section className="ded-section">
        <div className="ded-section-head">
          ...
          <table className="ded-table">
            ...
            <tbody>
              ...
              {stats.itemsTopValue.length === 0 && (
                <tr><td colSpan={5} className="ded-muted">No items yet.</td></tr>
              )}
            </tbody>
          </table>
        </div>
      ...
      <section className="ded-section">
        <div className="ded-section-head">
          <h2>ISK by Day</h2>
          <small className="ded-muted">Totals grouped by run date (local time) in current scope.</small>
        </div>
        <div className="ded-table-wrap">
          <table className="ded-table">
            <thead>
            ...
            <tbody>
              ...
              {stats.byDay.length === 0 && (
                <tr><td colSpan={5} className="ded-muted">No daily data yet.</td></tr>
              )}
            </tbody>
          </table>
        </div>
...
function computeStats(runs) {
  ...
  // By DED level (within scope)
  const levelMap = new Map();
  for (const r of runs) {
    const key = r.dedLevel || "Unknown";
    let obj = levelMap.get(key);
    if (!obj) { obj = { level: key, runs: 0, totalISK: 0, totalMin: 0 }; levelMap.set(key, obj); }
    obj.runs += 1;
  ...
  // Items aggregation (value + drop rate) within scope
  const itemAgg = new Map();
  for (const r of runs) {
    const present = new Set(); // for drop-rate per run
  ...
  const itemsTopValue = Array.from(itemAgg.values())
    .map(a => ({
      ...
      dropRate: totalRuns ? (a.runHits / totalRuns) : 0, // <-- denominator = runs in scope (e.g., only 5/10)
    }))
  ...
  for (const r of runs) {
    ...
    if (!obj) { obj = { day: d, runs: 0, totalISK: 0, totalMinutes: 0 }; dayMap.set(d, obj); }
    obj.runs += 1;
    obj.totalISK += num(r.iskTotal) || 0;
  ...
  return { totalRuns, totalMinutes, totalISK, avgISKPerRun, iskPerHour, avgMinutesPerRun, byLevel, itemsTopValue, byDay };
}
...
function fmtISK(v) { try { return `${new Intl.NumberFormat().format(Math.round(v))} ISK`; } catch { return `${Math.round(v)} ISK`; } }
function fmtInt(v) { try { return new Intl.NumberFormat().format(Math.round(v)); } catch { return String(Math.round(v)); } }
function fmtNum(v) { const n = Number(v); if (!Number.isFinite(n)) return "-"; return (Math.round(n * 10) / 10).toFixed(1); }
function fmtMinutes(totalMin) { const n = Number(totalMin) || 0; const h = Math.floor(n / 60); const m = Math.round(n % 60); return h <= 0 ? `${m} min` : `${h}h ${m}m`; }
function fmtPct(f) { const n = Number(f); if (!Number.isFinite(n)) return "-"; return `${(n * 100).toFixed(1)}%`; }
